...
const renderGallery = function (array, ul) {
  let NodeList = galleryList.map((value, index) => {
...
const moveInGallery = function (indexToMove) {
  if (indexToMove === 'right') {
    ...
    if (indexCurrenElement > galleryList.length - 1) {
      indexCurrenElement = 0;
    }
  } else if (indexToMove === 'left') {
    indexCurrenElement -= 1;
    if (indexCurrenElement < 0) {
      indexCurrenElement = galleryList.length - 1;
    }
  }
  changeImg(
...
renderGallery(galleryList, galleryRef);
galleryRef.addEventListener('click', getItemfromDOMGallery);
document
